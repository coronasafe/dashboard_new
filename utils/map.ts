...
interface ColorClassesArgs {
  currentCapacity: number;
  totalCapacity: number;
}

type ColorClasses = (args: ColorClassesArgs | undefined) => string;

export const colorClasses: ColorClasses = (capacity) => {
  if (capacity) {
    let percent = (capacity.currentCapacity / capacity.totalCapacity) * 100;
    if (percent < 70.0) {
...
interface CanShowBedArgs {
  capacity: { room_type: any; total_capacity: number };
  filter: string;
}
